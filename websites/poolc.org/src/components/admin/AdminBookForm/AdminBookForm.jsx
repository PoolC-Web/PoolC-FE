...
import { StyledActionButton, StyledForm, StyledInput, TitleContainer } from './AdminBookForm.styles';

const AdminBookForm = ({ onCreateBook, onUpdateBook, book, errorMessage, buttons, errorModalVisible, onCloseErrorModal }) => {
  const [title, onChangeTitle] = useInput(book ? book.title : '', notEmptyValidation);
  const [author, onChangeAuthor] = useInput(book ? book.author : '', notEmptyValidation);
  const [imageURL, setImageURL] = useState(book ? book.imageURL : '');
  const [info, onChangeInfo] = useInput(book ? book.info : '', notEmptyValidation);
    ...
    <>
      ...
      <WhiteNarrowBlock>
        ...
        <StyledForm>
          ...
          <FileUploadButton onSubmit={setImageURL} optimizeCardImage />
